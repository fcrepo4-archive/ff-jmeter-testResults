...
for (thread.count in thread.counts) {
  thread.files <- list.files(path=dir.path, pattern=sprintf("%s-threads",thread.count), all.files=F, full.names=T);
  for (file in thread.files) {
    filename<-sub(dir.path, '', file);
    print (filename);
    print (thread.count);
    add.create<-FALSE;

    # Pull out parts from the file name
    file.parts<-strsplit(filename, "-");
    label<-file.parts[[1]][2];
    mean<-as.numeric(file.parts[[1]][4]);
    stddev<-file.parts[[1]][6];
    #title <- tail(strsplit(file, "/")[[1]], 1);
    bin.width<-as.numeric(stddev) * 6/50;
    #cat("label ", label, "mean ", mean, "stddev ", stddev, "bin width ", bin.width, "\n");
    ...
    # Filter data - interested in read and create Datastream
    binReadData <- data[data[,'label'] == "Read Datastream",];
    binCreateData <- data[data[,'label'] == "Create Datastream",];
...
#print(data.all, quote = TRUE, row.names = FALSE)

# Transforming the data
data.all$fileSize <- as.numeric(data.all$fileSize);
data.all$threadCount <- as.numeric(data.all$threadCount);

data.all <- transform(data.all, label = factor(label, levels=c("Delete Object", "Create Object", "Get Object", "Create Datastream", "Update Datastream", "Read Datastream"), labels=c("Delete object", "Create object", "Read object", "Create data", "Update data", "Read data")))
